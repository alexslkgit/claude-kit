#!/usr/bin/env bash
# context-guard.sh — makes the clear-at-250k rule fire by itself.
#
...
#   a session starts at a 65k floor and grows ~26k per user message (p90 84k)
#   cost per request: 21k below 150k · 33k at 200–250k · 46k at 300–400k · 94k past 500k
#   a handoff costs ~200k units and breaks even after 11 requests — the median user message
#   is 25 requests, so past 250k the clear pays for itself inside one message of work
#   250k, not the 160k token optimum: 160k arrives after 3.7 messages and buys only 16%
#
# Two bands, and the difference between them matters:
#   220k  soft — do not start anything large, finish what is open. Said once.
#   250k  hard — wrap up at the next boundary and tell him to press /clear. Repeats.
#
...
rule, measured and set 2026-08-17, is to stop past 250k at the next natural boundary — a finished
...
EOF
  ...
  cat <<EOF
context-guard: this conversation is at ${k}k tokens, ~${per_req}k units per request. Past 250k the
