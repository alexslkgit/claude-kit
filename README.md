...
| `inbox/` | the owner's queue: `ask.sh` writes one request, `server.mjs` serves them all on http://localhost:7654. Every session on the machine writes to the same queue, he answers in one place, and `--wait` returns his click to the session that is blocked on it. `--steps "one|two|three"` renders as a checklist instead of one wall of `--why` text, ticked off as he works. Every URL, path and backtick span in `why`/`steps` becomes its own copyable chunk |
...
change by the `board` skill. Reasoning stays in the journal; the board carries only the
conclusion.
